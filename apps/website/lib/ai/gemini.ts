...
/**
 * Executes a server-side request to Google Gemini API.
...
export async function generateGeminiContent(options: GeminiGenerateOptions): Promise<string> {
  ...
  if (!apiKey) {
    // Fallback response when key is not configured locally
    return `[DragonOS AI Engine] Processed prompt: "${options.prompt.slice(0, 60)}...". (Configure GEMINI_API_KEY in .env for live Google Gemini LLM API calls).`;
  }
  ...
  try {
    ...
    return "No text response generated by Gemini API.";
  } catch (error: any) {
    console.error("Gemini Request Exception:", error);
    return `[DragonOS AI Fallback] Unable to complete Gemini request: ${error.message}`;
  }
}
